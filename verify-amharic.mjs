...
// Run with: node verify-amharic.mjs
...
try {
    ...
    if (amSection) {
        ...
        // Check for all prompt IDs - need to look in the prompts section specifically
        const promptsSection = amSection.split('"prompts": {')[1];
        ...
        if (promptsSection) {
            promptIds.forEach(id => {
                const pattern = `"${id}": {`;
                const found = promptsSection.includes(pattern);
                console.log(found ? `  ✅ ${id}` : `  ❌ ${id} missing`);
                if (!found) allFound = false;
            });
        } else {
            console.log('  ❌ Could not find prompts section');
            allFound = false;
        }
    ...
    // Check file size
    const stats = statSync('src/lib/i18n.ts');
